...
class ProductoDatabase {
  ...
  insertarProductosDb(ProductosData productos) async {
    try {
      ...
      final res = await db.rawInsert('INSERT OR REPLACE INTO Producto (id_producto,id_categoria,producto_nombre,producto_foto,producto_orden,'
  ...
  Future<List<ProductosData>> consultarPorId(String id) async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE id_producto='$id' and producto_estado='1' ");

    List<ProductosData> list = res.isNotEmpty ? res.map((c) => ProductosData.fromJson(c)).toList() : [];
  ...
  Future<List<ProductosData>> consultarPorQueryLocal(String query) async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE producto_nombre LIKE '%$query%' and producto_estado='1' and producto_carta='1' ");
  ...
  Future<List<ProductosData>> consultarPorQueryDelivery(String query) async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE producto_nombre LIKE '%$query%' and producto_estado='1' and producto_delivery='1'");
  ...
  Future<List<ProductosData>> obtenerProductosPorCategoriaLocal(String id) async {
    final db = await dbprovider.database;
  ...
  Future<List<ProductosData>> obtenerProductosPorCategoriaDelivery(String id) async {
    final db = await dbprovider.database;
    final res = await db.rawQuery(
  ...
  Future<List<ProductosData>> obtenerPropinas(String id) async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE id_categoria='$id'  order by CAST(producto_orden AS INT) ASC");
  ...
  Future<List<ProductosData>> obtenerProductosDestacados() async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE producto_estado_destacado='1'  order by CAST(producto_destacado AS INT) ASC");
  ...
  Future<List<ProductosData>> obtenerFavoritos() async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Producto WHERE producto_favorito = 1 and producto_estado='1' ");
