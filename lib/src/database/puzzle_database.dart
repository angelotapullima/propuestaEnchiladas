...
class PuzzleDatabase {
  ...
  insertarPuzzle(PuzzleDatum puzzle) async {
    try {
      ...
      final res = await db
          .rawInsert("INSERT OR REPLACE INTO Puzzle (id_imagen,imagen_ruta,imagen_titulo,imagen_subida,imagen_inicio,imagen_fin,imagen_estado) "
  ...
  Future<List<PuzzleDatum>> consultarPuzzle() async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Puzzle WHERE imagen_estado='1'");

    List<PuzzleDatum> list = res.isNotEmpty ? res.map((c) => PuzzleDatum.fromJson(c)).toList() : [];
  ...
  Future<List<PuzzleDatum>> obtenerPuzzle() async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Puzzle ");
